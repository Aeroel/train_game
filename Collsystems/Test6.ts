 ...
 */
function resolveCollisions(player: Entity, walls: Entity[], dt: number): void {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    let collisionDirection: 'x' | 'y' | null = null;
    let nearestWall: Entity | null = null;
...
function runTest(testName: string, player: Entity, walls: Entity[], expectedPlayer: { x: number, y: number }, timeStep: number = 1): void {
  console.log(`\n=== ${testName} ===`);
  ...
  const wallsPosPass = wallFinals.every((wallFinal, i) =>
    Math.abs(initialWalls[i].x + initialWalls[i].vx * timeStep - wallFinal.x) < 0.001 &&
...
runTest(
  ...
  { x: 0, y: 0, vx: 100, vy: 0, w: 10, h: 10 },
  [{ x: 50, y: 0, vx: 0, vy: 0, w: 10, h: 10 }],
  { x: 40, y: 0 }
);

runTest(
  'Test 2 - Moving wall, standing player',
  { x: 0, y: 0, vx: 0, vy: 0, w: 10, h: 10 },
  [{ x: -100, y: 0, vx: 500, vy: 0, w: 10, h: 10 }],
  { x: 410, y: 0 }
);

runTest(
  'Test 3 - Both move towards each other',
  { x: 100, y: 0, vx: -100, vy: 0, w: 10, h: 10 },
  [{ x: 50, y: 0, vx: 100, vy: 0, w: 10, h: 10 }],
  { x: 160, y: 0 }
);

runTest(
  'Test 4 - High velocity diagonal movement',
  { x: 0, y: 0, vx: 100000, vy: 100000, w: 10, h: 10 },
  [{ x: 150, y: 150, vx: 0, vy: 0, w: 10, h: 10 }],
  { x: 100000, y: 140 }
);
...
runTest(
  ...
  { x: 0, y: 0, vx: 100000, vy: 100000, w: 10, h: 10 },
  [{ x: 140, y: 140, vx: -10, vy: -10, w: 10, h: 10 }, { x: 5000, y: 140, vx: 0, vy: 0, w: 10, h: 10 }],
  { x: 4990, y: 140 }
);

runTest(
  'Test 6 - Corner case stationary walls',
  { x: 6, y: 5, vx: 1000, vy: 1000, w: 1, h: 1 },
  [{ x: 0, y: 0, vx: 0, vy: 0, w: 2, h: 4 }, { x: 0, y: 0, vx: 0, vy: 0, w: 5, h: 4 }],
  { x: 2, y: 2 }
);

runTest(
  'Test 7 - Corner case moving walls',
  { x: 6, y: 5, vx: 10000, vy: 10000, w: 1, h: 1 },
  [{ x: 0, y: 0, vx: 100, vy: 100, w: 2, h: 4 }, { x: 0, y: 0, vx: 100, vy: 100, w: 5, h: 4 }],
  { x: 102, y: 102 }
);