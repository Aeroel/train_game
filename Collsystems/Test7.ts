...
function collidePlayerWithWalls(player: Entity, walls: Entity[]): void {
    ...
    while (remainingTime > 0) {
        let earliestCollision: { t: number, normalX: number, normalY: number, wall: Entity } | null = null;

        for (const wall of walls) {
            const relativeVx = currentVx - wall.vx;
            const relativeVy = currentVy - wall.vy;

            let entryX: number, exitX: number;
            if (relativeVx > 0) {
            ...
            let entryY: number, exitY: number;
            if (relativeVy > 0) {
        ...
        if (wall.vx === 0 && wall.vy === 0) {
            if (normalX !== 0) currentVx = 0;
            if (normalY !== 0) currentVy = 0;
        } else {
            currentVx = wall.vx;
            currentVy = wall.vy;
        }
...
function testCollisionSystem() {
    function runTest(testName: string, player: Entity, walls: Entity[], expectedPlayer: {x: number, y: number}, expectedWalls: {x: number, y: number}[]) {
