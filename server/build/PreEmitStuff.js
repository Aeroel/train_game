...
class PreEmitStuff {
    ...
    static get_visible_to_player_entities_and_virtual_width_and_virtual_height_and_hide_all_the_real_world_xy_coordinates_by_returning_virtual_ones_instead(player) {
        ...
        return {
            entities: entitiesWithOnlyProperties,
    ...
    static getVisiblePortion(entityWhichIsLookingAround, possiblyVisibleEntity) {
        ...
        const visibleX = Math.max(visionArea.x, possiblyVisibleEntity.x);
        const visibleY = Math.max(visionArea.y, possiblyVisibleEntity.y);
        const visibleWidth = Math.min(visionArea.x + visionArea.width, possiblyVisibleEntity.x + possiblyVisibleEntity.width) - visibleX;
        const visibleHeight = Math.min(visionArea.y + visionArea.height, possiblyVisibleEntity.y + possiblyVisibleEntity.height) - visibleY;
    ...
    ;
    static getVisibleEdges(entityWhichIsLookingAround, possiblyVisibleEntity) {
        // Calculate the player's vision area based on visionRange and centered position
        const visionCenterX = entityWhichIsLookingAround.x + entityWhichIsLookingAround.width / 2;
        const visionCenterY = entityWhichIsLookingAround.y + entityWhichIsLookingAround.height / 2;
        const visionArea = {
