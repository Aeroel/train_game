...
function rectsOverlap(a: SimpEntity, b: SimpEntity): boolean {
  return !(
    a.x + a.width <= b.x ||
    a.x >= b.x + b.width ||
    a.y + a.height <= b.y ||
 ...
 */
export function resolvePlayerVsWalls(
  playerIn: Base_Entity): { player: SimpEntity; walls: SimpEntity[] } {
  const player = clone({
   x: playerIn.x,
   y: playerIn.y,
   vy: playerIn.vy,
   vx: playerIn.vx,
   width: playerIn.width,
   height: playerIn.height,
    
  });
  const walls = World.filterEntities(ent=>ent.hasTag("Wall")).map(w=>clone({
     x: w.x,
   y: w.y,
   vy: w.vy,
   vx: w.vx,
  ...
  for (let iter = 0; iter < 16 && remain > EPS; iter++) {
    ...
    for (let i = 0; i < wInit.length; i++) {
      ...
      const wallSnapshot: SimpEntity = { x: wx, y: wy, vx: 0, vy: 0, width: w.width, height: w.height };
 ...
 const wallsCast = wallsFinal as unknown as  SimpEntity[];
 playerIn.x= player.x
 playerIn.y= player.y
  return { player, walls: wallsCast };
}
